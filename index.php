...
require("geographic/vendor/autoload.php");
...
// load config...
Config::load('./geographic/config.php');

// custom config file to be written to by a bash script or something
Config::load('./geographic/custom_config.php');
...
foreach (Config::get('plugins', array()) as $plugin) {
    ...
    if (file_exists('./geographic/plugins/' . $plugin_class . '.php')) {

        // use user plugin from /plugins/
        require_once('./geographic/plugins/' . $plugin_class . '.php');
