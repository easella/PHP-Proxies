...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>miniProxy</title><script async src=\"https://arc.io/widget.min.js#zV44DMAX\"></script><link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css\"/><style>button{cursor:pointer;}</style></head><body><center><h1>Welcome to miniProxy!</h1>miniProxy can be directly invoked like this: <a href=\"" . PROXY_PREFIX . $landingExampleURL . "\">" . PROXY_PREFIX . $landingExampleURL . "</a><br /><br />Or, you can simply enter a URL below:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" class=\"form-control text-center\" size=\"50\" /><button class=\"btn btn-primary\" title=\"Proxy It!\" type=\"submit\" style=\"width:100%;\">Proxy It!</button></form></center></body></html>");
    } else {
      $url = $startURL;
    }
